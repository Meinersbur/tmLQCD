...
static int msg_InjFifoInit ( msg_InjFifoHandle_t *injFifoHandlePtr,
 ...
 */
static uint64_t msg_InjFifoInject ( msg_InjFifoHandle_t injFifoHandle,
...
// alltoall
static void setup_mregions_bats_counters(void);
...
//Call GI barrier
static void global_barrier(void)
{
  int rc = 0;
...
static unsigned msg_InjFifoCheckCompletion(msg_InjFifoHandle_t injFifoHandle,
                                    uint32_t            relativeFifoId,
                                    uint64_t            desc_count);
...
static int main2(int argc, char **argv)
{
    ...
    uint64_t totalBytesPerIter     =  messageSizeInBytes * (COMMDIR_COUNT - 1);
...
static ucoord mySpirank;//Required
static inline ucoord bgq_abcde2spirank(Personality_t *pers, uint8_t a, uint8_t b,uint8_t c,uint8_t d,uint8_t e) {
	assert(pers);
	torus_t dims =
	    {
	...
	ucoord result = ((((a)*dims.b + b)*dims.c + c)*dims.d + d)*dims.e + e;
...
static void bgq_comm_test(bool nospi) {
			...
			for (ucoord v = 0; v < 2; v += 1){
				for (ucoord c = 0; c < 3; c += 1){
					for (ucoord k = 0; k < 2; k += 1){
						g_bgq_sec_send[d][i].s[v][c][k] = (double) g_cart_id;
					}
				}
			}
		}
	...
	for (ucoord d = 0; d < PHYSICAL_LD; d += 1) {
		if (!bgq_direction_isDistributed(d))
			continue;
			...
			for (ucoord v = 0; v < 2; v += 1) {
				for (ucoord c = 0; c < 3; c += 1) {
					for (ucoord k = 0; k < 2; k += 1) {
						...
						if (val != rank_neighbor) {
							printf("Exchange doesn't work for dir %d: %d != %f at point %d\n", d, rank_neighbor, creal(val), i);
							exit(1);
						}
					}
				}
...
void bgq_comm_spi_init(void) {
	...
	int spi_num_dirs = 2*(COMM_T+COMM_X+COMM_Y+COMM_Z);
	assert(spi_num_dirs==COMMDIR_COUNT);
	...
	for (ucoord d_src = 0; d_src < PHYSICAL_LD; d_src+=1) {
		...
		roffsets[commdir_dst] = bgq_weyl_section_offset(sec_recv) - bgq_weyl_section_offset(sec_recv_begin);
		assert(roffsets[commdir_dst] % BGQ_ALIGNMENT_L2 == 0);
		assert((roffsets[commdir_dst] + secsize) <= (bgq_weyl_section_offset(sec_recv_end) - bgq_weyl_section_offset(sec_recv_begin)));
	...
	// Set up base address table for reception counter and buffer
	assert(bgq_weyl_section_offset(sec_recv_end) - bgq_weyl_section_offset(sec_recv_begin) == totalMessageSize);
	setup_mregions_bats_counters(bgq_weyl_section_offset(sec_recv_end) - bgq_weyl_section_offset(sec_recv_begin));
