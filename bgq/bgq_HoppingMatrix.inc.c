		...
		for (int tv = 0; tv < PHYSICAL_LTV; tv+=1)
			for (int y = 0; y < LOCAL_LY; y+=1)
				for (int z = 0; z < LOCAL_LY; z+=1) {
					int x = (d==XUP) ? -1 : LOCAL_LX;
					const int t1 = ((isOdd+x+y+z)&1)+tv*PHYSICAL_LP*PHYSICAL_LK;
					const int t2 = t1 + 2;

					bgq_weylsite *weylsite = BGQ_WEYLSITE_X(weylxchange_send[d], !isOdd, tv, x, y, z, t1, t2, false, false);
					for (int i = 0; i < 2; i+=1)
						for (int l = 0; l < 3; l+=1) {
							weylsite->s[i][l][0] = t1 + d*_Complex_I;
							weylsite->s[i][l][0] = t2 + d*_Complex_I;
						}
				}
	#endif
