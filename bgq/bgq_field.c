...
static bgq_weylfield_section bgq_HoppingMatrix_init_source_sectionof(bool isOdd, size_t tv, size_t x, size_t y, size_t z, bgq_direction d) {
	size_t t1 = bgq_physical2t1(isOdd, tv, x,y,z);
	size_t t2 = bgq_physical2t2(isOdd, tv, x,y,z);

	if (COMM_T && (t2==LOCAL_LT-1) && (d==TUP)) {
		return sec_recv_tup;
	}
	if (COMM_T && (t1==0) && (d==TDOWN)) {
	...
	for (size_t isOdd = false; isOdd <= true; isOdd+=1) {
		...
		for (size_t ih = 0; ih < PHYSICAL_VOLUME; ih+=1) {
			...
			for (size_t pd_cnt = P_TUP1; pd_cnt <= P_ZDOWN; pd_cnt+=1) {
				...
				bgq_weylfield_section sec = bgq_HoppingMatrix_init_source_sectionof(isOdd, tv, x, y, z, d);
	...
	for (size_t isOdd_cnt = false; isOdd_cnt <= true; isOdd_cnt+=1) {
		...
		for (size_t ih_src = 0; ih_src < PHYSICAL_VOLUME; ih_src+=1) {
			...
			for (size_t pd_cnt = P_TUP1; pd_cnt <= P_ZDOWN; pd_cnt+=1) {
				...
				bgq_weylfield_section sec_src = bgq_HoppingMatrix_init_source_sectionof(isOdd_dst, tv_src, x_src, y_src, z_src, d_src);
				bgq_weylfield_section sec_dst = bgq_HoppingMatrix_init_source_sectionof(isOdd_dst, tv_dst, x_dst, y_dst, z_dst, d_dst);
				bgq_weylfield_section sec_src_send = bgq_section_commbuftran(sec_dst, true);
				...
				bgq_weylfield_section expectedSec = bgq_sectionOfOffset(offset_dst);
				assert(bgq_weyl_section_offset(sec_dst) <= offset_dst && offset_dst < bgq_weyl_section_offset(sec_dst+1));
				size_t offset_dst_write;
				if ((sec_src==sec_body) || (sec_src==sec_surface)) {
					// No communication, do everything locally
					assert((sec_src_send==sec_body) || (sec_dst==sec_src_send));
					offset_dst_write = offset_dst;
				} else {
					// Communication with MPI buffers, translate recv- to send-buffer
					assert((sec_dst!=sec_body) && (sec_dst!=sec_surface));
					size_t relOffset = bgq_weyl_section_offset(sec_dst)/*recv*/ - offset_dst;
					offset_dst_write = bgq_weyl_section_offset(sec_src_send) + relOffset;
				}

				g_bgq_destoffset_fromHalfvolume[isOdd_src][ih_src].pd[pd_src] = bgq_encode_offset(offset_dst_write);
				if (isSurface) {
					size_t is_src = bgq_halfvolume2surface(isOdd_src, ih_src);
					g_bgq_destoffset_fromSurface[isOdd_src][is_src].pd[pd_src] = bgq_encode_offset(offset_dst_write);
				} else {
					size_t ib_src = bgq_halfvolume2body(isOdd_src, ih_src);
					g_bgq_destoffset_fromBody[isOdd_src][ib_src].pd[pd_src] = bgq_encode_offset(offset_dst_write);
				}
			}
		}
	}
...
static void bgq_spinorfield_fullspinor2weyl_worker(void *args_untyped, size_t tid, size_t threads) {
	...
	for (size_t ih = begin; ih < end; ih += 1) {
		...
		bgq_weyl_ptr_t *weylptrs = &field->destptrFromHalfvolume[ih];
...
void bgq_spinorfield_setup(bgq_weylfield_controlblock *field, bool isOdd, bool readFullspinor, bool writeFullspinor, bool readWeyl, bool writeWeyl) {
	...
	if (field->isInitinialized) {
		if (field->isOdd == isOdd) {
			...
			master_print("Performance loss by reuse of spinorfield with different oddness\n");
			initDestPtrs = true;
	...
	if (initDestPtrs) {
		...
		for (size_t is = 0; is < PHYSICAL_SURFACE; is += 1) {
			for (size_t pd_cnt = 0; pd_cnt < P_COUNT; pd_cnt += 1) {
				...
				size_t offset = bgq_decode_offset(g_bgq_offset_fromSurface[isOdd][is].pd[pd]);
				assert(offset+1);
	...
	if (readWeyl) {
		if (weylAvailable) {
			...
			// Do a conversion
			master_print("Warning: Converted fullspinor layout to weyl layout; For best performance, this should have been done in the first place\n");
