...
#undef BGQ_GAUGESITE
#define BGQ_GAUGESITE(gaugefield,isOdd,tv,x,y,z,dir,t1,t2,isRead,isWrite) (bgq_gaugesite*)(isRead ? /*read*/(tmp=gaugeaddr,gaugeaddr+=2*3*3*2,tmp) : (isWrite ? /*write*/(tmp=writeaddr,writeaddr+=2*3*3*2,tmp) : /*prefetch*/(gaugeaddr) ) )
...
//#pragma omp parallel
	{
		...
		PRECISION *gaugeaddr = ((PRECISION*) gaugefield) + (size_t)tid * (8*9*2) / threads;
		gaugeaddr = (PRECISION*) ((size_t) gaugeaddr & ~(BGQ_PRECISION - 1)); // alignment
...
//#pragma omp for schedule(static)
		...
		for (size_t counter = 0; counter < threadcount; counter += 1) {
			...
			WORKLOAD_DECL(txyz,count);
			const size_t tv = WORKLOAD_PARAM(PHYSICAL_LTV);
			const size_t x = WORKLOAD_PARAM(PHYSICAL_LX);
			const size_t y = WORKLOAD_PARAM(PHYSICAL_LY);
			const size_t z = WORKLOAD_PARAM(PHYSICAL_LZ);
			//WORKLOAD_CHECK

			const size_t t1 = tv * PHYSICAL_LP * PHYSICAL_LK + ((isOdd + x + y + z) & 1);
			const size_t t2 = t1 + PHYSICAL_LP;

			{
				...
				// T+ //////////////////////////////////////////////////////////////////////////
				{
					...
					//bgq_su3_spinor_invalidate(spinorsite_tup);
					...
					//bgq_su3_matrix_invalidate(gaugesite_tup);
				...
				// T- //////////////////////////////////////////////////////////////////////////
				{
					...
					//bgq_su3_spinor_invalidate(spinorsite_tdown);
					...
					//bgq_su3_matrix_invalidate(gaugesite_tdown);
